...
def sync_project(project_info, remote_info):
    while True:
        sync_project_once(project_info, remote_info)
        wait_for_updates(project_info)


def sync_project_once(project_info, remote_info):
...
def wait_for_updates(project_info):
    ...
    inotifywait_command = [f'inotifywait',
                           ...
                           #f'1>/dev/null']
    try:
        subprocess.run(inotifywait_command, check=True, stderr=subprocess.DEVNULL)
    except subprocess.CalledProcessError as e:
